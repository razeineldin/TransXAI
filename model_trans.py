...
from loss import custom_loss, generalized_dice
...
def transxai_model(input_shape, filter_num, n_labels, stack_num_down=2, stack_num_up=2,
                 embed_dim=768, num_mlp = 3072, num_heads=12, num_transformer=12,
                 activation='ReLU', mlp_activation='GELU', output_activation='Softmax',
                 batch_norm=False, pool=True, unpool=True, backbone=None, weights='imagenet',
                 freeze_backbone=True, freeze_batch_norm=True, name='transunet',
                 learning_rate=0.01, saved_model_dir=None):
    activation_func = eval(activation)
    ...
    model.compile(optimizer=sgd, loss=custom_loss, metrics=[generalized_dice])
